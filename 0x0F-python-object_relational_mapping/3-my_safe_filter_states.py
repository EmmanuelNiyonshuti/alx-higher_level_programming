...
if __name__ == "__main__":
    ...
    """Using parameterized queries make it a little safer
    from MySQL injection"""
    curr.execute("SELECT * FROM states WHERE name = %s ORDER BY states.id ASC", (state_name, ))
    rows = curr.fetchmany(size=1)
    for row in rows:
        print(row)
